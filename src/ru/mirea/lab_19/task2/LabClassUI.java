...
public class LabClassUI extends JFrame {
    ...
    public LabClassUI(ArrayList<Student> students){
        ...
        buttonsPanel.add(remStudentBtn);
        buttonsPanel.add(findStudentBtn);
    ...
    private void addBtnClicked() throws IllegalArgumentException {
        ...
        String age = JOptionPane.showInputDialog("Возраст");
        String group = JOptionPane.showInputDialog("Номер группы:");
        String avg = JOptionPane.showInputDialog("Средний балл:");

        int enteredAvg;
        int enteredGroup;
        int enteredAge;
        try {
            enteredAvg = Integer.parseInt(avg);
            enteredGroup = Integer.parseInt(group);
            enteredAge = Integer.parseInt(age);
        } catch (NumberFormatException e){
            throw new NumberFormatException("Невозможно преобразовать строку " +
                    e.getMessage().substring(17) +
                    " в число!");
        }

        students.add(new Student(fio, enteredAvg, enteredGroup, enteredAge));
